...
par()
a + geom_line() + geom_point() + scale_x_discrete(breaks = table2$axt, labels = substr(table2$axt, 1, 4)) + ylab("Craving for Nicotine (Mean)") + scale_colour_discrete(name = "Group",  breaks = c(1:4), labels = c("no smoking + placebo patch", "no smoking + active patch", "smoking + placebo patch", "smoking + active patch")) + theme(legend.position = "bottom")
```
