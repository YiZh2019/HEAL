...
session1_long <- session1 %>% select(., c(sid, axtvar)) %>% gather(., key = variable, value = value, axtvar) %>% separate(variable, c("axt", "variable"), sep = "_", extra = "merge") %>% spread(., variable, value) %>% select(., sid, axt, contains("mnws")) %>% arrange(sid, axt)
