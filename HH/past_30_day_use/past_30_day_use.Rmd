...
select(data, all_of(w1_8)) %>% apply(., 2, range, na.rm = T)
select(data, all_of(w9)) %>% apply(., 2, range, na.rm = T)

table(data$W9_Heroin_Use_Last30a)
table(data$W9_Heroin_Use_Last30)
table(data$W9_Alc_Use_Last30a)
table(data$W9_Alc_Use_Last30)
data[, c("W9_Alc_Use_Last30", "W9_Alc_Use_Last30a")]
```
