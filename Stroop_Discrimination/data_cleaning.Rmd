...
remove.list <- c("ALWAYS", "ELEMENT", "FEBRUARY", "INJUSTICE", "INEQUALITY")
data <- mergedata[!mergedata$Target %in% remove.list, ]
...
# 
origin %>% filter(., Session == 2) %>% group_by(SID, Wordtype_RC, Target) %>% summarise(n = n()) %>% filter(., n != 3)
