---
title: "09302019 demo PP.concierge"
author: "Yi"
date: "10/9/2019"
output: pdf_document
---
...
PP.concierge <- data
dim(PP.concierge)
colnames(PP.concierge)
length(unique(PP.concierge$redcapid))
```
...
# gender
FrequencyPercentage(PP.concierge$gender.factor)

# Ethnicity
FrequencyPercentage(PP.concierge$eth.factor)

# race
miss.race <- NROW(which(PP.concierge$race___1 == 0 & PP.concierge$race___2 == 0 & PP.concierge$race___3 == 0 & PP.concierge$race___4 == 0 & PP.concierge$race___5 == 0 & PP.concierge$race___6 == 0 & PP.concierge$race___7 ==0))

total.race <- dim(PP.concierge)[1] - miss.race
...
RaceDescrip(PP.concierge$race___1.factor)

RaceDescrip(PP.concierge$race___2.factor)

RaceDescrip(PP.concierge$race___3.factor)

RaceDescrip(PP.concierge$race___4.factor)

RaceDescrip(PP.concierge$race___5.factor)

RaceDescrip(PP.concierge$race___6.factor)

RaceDescrip(PP.concierge$race___7.factor)

# Highest level of education
FrequencyPercentage(PP.concierge$degree_program.factor)

# currently enrolled in a program
freq.degree.type <- table(PP.concierge$degree_type.factor)
perc.degree.type <- round(freq.degree.type/sum(PP.concierge$current_degree_program == 1, na.rm = T) * 100,1)
rbind(freq.degree.type, perc.degree.type)

# own or rent home
FrequencyPercentage(PP.concierge$own_rent.factor)

# currently living situation
FrequencyPercentage(PP.concierge$home.factor)

# current financial situation
FrequencyPercentage(PP.concierge$overall_income.factor)

# childhood financial situation
FrequencyPercentage(PP.concierge$family_income.factor)

# Country of residence
FrequencyPercentage(PP.concierge$country.factor)
...
num.first.fail <- sum(!(PP.concierge$att_check1___1 == 1 & PP.concierge$att_check1___4 == 1 & PP.concierge$att_check1___5 == 1))

num.second.fail <- sum(PP.concierge$last_30_ecig_yn == 1 & PP.concierge$att_check2 != 3, na.rm = T)

num.third.fail <- sum(!(PP.concierge$att_check3a == 4 & grepl("decision making", PP.concierge$att_check3b, ignore.case = T)))
...
PP.concierge <- mutate(PP.concierge, att1.success = ifelse(PP.concierge$att_check1___1 == 1 & PP.concierge$att_check1___4 == 1 & PP.concierge$att_check1___5 == 1, 1, 0))
PP.concierge$att1.success.factor <- factor(PP.concierge$att1.success, levels = 0:1)
levels(PP.concierge$att1.success.factor) <- c("first attention checks failed", "first attention checks successed")

PP.concierge <- mutate(PP.concierge, att2.success = ifelse(PP.concierge$last_30_ecig_yn == 1 & PP.concierge$att_check2 == 3, 1, ifelse(PP.concierge$last_30_ecig_yn == 1, 0, NA)))
PP.concierge$att2.success.factor <- factor(PP.concierge$att2.success, levels = 0:1)
levels(PP.concierge$att2.success.factor) <- c("second attention checks failed", "second attention checks successed")

PP.concierge <- mutate(PP.concierge, att3.success = ifelse(PP.concierge$att_check3a == 4 & grepl("decision making", PP.concierge$att_check3b, ignore.case = T), 1, 0))
PP.concierge$att3.success.factor <- factor(PP.concierge$att3.success, levels = 0:1)
levels(PP.concierge$att3.success.factor) <- c("third attention checks failed", "third attention checks successed")

PP.concierge$temp.att2 <- PP.concierge$att2.success
PP.concierge$temp.att2[is.na(PP.concierge$att2.success)] = 1
sum.att.success <- PP.concierge$att1.success + PP.concierge$temp.att2 + PP.concierge$att3.success
