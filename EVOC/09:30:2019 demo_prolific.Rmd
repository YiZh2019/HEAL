...
# gender
FrequencyPercentage(prolific$gender.factor)

# Ethnicity
FrequencyPercentage(prolific$eth.factor)

# race
NROW(which(prolific$race___1 == 0 & prolific$race___2 == 0 & prolific$race___3 == 0 & prolific$race___4 == 0 & prolific$race___5 == 0 & prolific$race___6 == 0 & prolific$race___7 ==0))

FrequencyPercentage(prolific$race___1.factor)

FrequencyPercentage(prolific$race___2.factor)

FrequencyPercentage(prolific$race___3.factor)

FrequencyPercentage(prolific$race___4.factor)

FrequencyPercentage(prolific$race___5.factor)

FrequencyPercentage(prolific$race___6.factor)

FrequencyPercentage(prolific$race___7.factor)

# Highest level of education
FrequencyPercentage(prolific$degree_program.factor)

# currently enrolled in a program
freq.degree.type <- table(prolific$degree_type.factor)
perc.degree.type <- round(freq.degree.type/sum(prolific$current_degree_program == 1) * 100,1)
rbind(freq.degree.type, perc.degree.type)

# own or rent home
FrequencyPercentage(prolific$own_rent.factor)

# currently living situation
FrequencyPercentage(prolific$home.factor)

# current financial situation
FrequencyPercentage(prolific$overall_income.factor)

# childhood financial situation
FrequencyPercentage(prolific$family_income.factor)

# Country of residence
FrequencyPercentage(prolific$country.factor)
...
num.first.fail <- sum(!(prolific$att_check1___1 == 1 & prolific$att_check1___4 == 1 & prolific$att_check1___5 == 1))

num.second.fail <- sum(prolific$last_30_ecig_yn == 1 & prolific$att_check2 != 3, na.rm = T)

num.third.fail <- sum(!(prolific$att_check3a == 4 & grepl("decision making", prolific$att_check3b, ignore.case = T)))
...
prolific <- mutate(prolific, att1.success = ifelse(prolific$att_check1___1 == 1 & prolific$att_check1___4 == 1 & prolific$att_check1___5 == 1, 1, 0))
prolific$att1.success.factor <- factor(prolific$att1.success, levels = 0:1)
levels(prolific$att1.success.factor) <- c("first attention checks failed", "first attention checks successed")

prolific <- mutate(prolific, att2.success = ifelse(prolific$last_30_ecig_yn == 1 & prolific$att_check2 == 3, 1, ifelse(prolific$last_30_ecig_yn == 1, 0, NA)))
prolific$att2.success.factor <- factor(prolific$att2.success, levels = 0:1)
levels(prolific$att2.success.factor) <- c("second attention checks failed", "second attention checks successed")

prolific <- mutate(prolific, att3.success = ifelse(prolific$att_check3a == 4 & grepl("decision making", prolific$att_check3b, ignore.case = T), 1, 0))
prolific$att3.success.factor <- factor(prolific$att3.success, levels = 0:1)
levels(prolific$att3.success.factor) <- c("third attention checks failed", "third attention checks successed")

prolific$temp.att2 <- prolific$att2.success
prolific$temp.att2[is.na(prolific$att2.success)] = 1
sum.att.success <- prolific$att1.success + prolific$temp.att2 + prolific$att3.success
