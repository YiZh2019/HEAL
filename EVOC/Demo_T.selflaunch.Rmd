---
title: "09302019 demo T.selflaunch"
author: "Yi"
date: "10/8/2019"
output: pdf_document
---
...
T.selflaunch <- data
dim(T.selflaunch)
colnames(T.selflaunch)
length(unique(T.selflaunch$redcapid))
```
...
# gender
FrequencyPercentage(T.selflaunch$gender.factor)

# Ethnicity
FrequencyPercentage(T.selflaunch$eth.factor)

# race
miss.race <- NROW(which(T.selflaunch$race___1 == 0 & T.selflaunch$race___2 == 0 & T.selflaunch$race___3 == 0 & T.selflaunch$race___4 == 0 & T.selflaunch$race___5 == 0 & T.selflaunch$race___6 == 0 & T.selflaunch$race___7 ==0))

total.race <- dim(T.selflaunch)[1] - miss.race
...
RaceDescrip(T.selflaunch$race___1.factor)

RaceDescrip(T.selflaunch$race___2.factor)

RaceDescrip(T.selflaunch$race___3.factor)

RaceDescrip(T.selflaunch$race___4.factor)

RaceDescrip(T.selflaunch$race___5.factor)

RaceDescrip(T.selflaunch$race___6.factor)

RaceDescrip(T.selflaunch$race___7.factor)

# Highest level of education
FrequencyPercentage(T.selflaunch$degree_program.factor)

# currently enrolled in a program
freq.degree.type <- table(T.selflaunch$degree_type.factor)
perc.degree.type <- round(freq.degree.type/sum(T.selflaunch$current_degree_program == 1, na.rm = T) * 100,1)
rbind(freq.degree.type, perc.degree.type)

# own or rent home
FrequencyPercentage(T.selflaunch$own_rent.factor)

# currently living situation
FrequencyPercentage(T.selflaunch$home.factor)

# current financial situation
FrequencyPercentage(T.selflaunch$overall_income.factor)

# childhood financial situation
FrequencyPercentage(T.selflaunch$family_income.factor)

# Country of residence
FrequencyPercentage(T.selflaunch$country.factor)
...
num.first.fail <- sum(!(T.selflaunch$att_check1___1 == 1 & T.selflaunch$att_check1___4 == 1 & T.selflaunch$att_check1___5 == 1))

num.second.fail <- sum(T.selflaunch$last_30_ecig_yn == 1 & T.selflaunch$att_check2 != 3, na.rm = T)

num.third.fail <- sum(!(T.selflaunch$att_check3a == 4 & grepl("decision making", T.selflaunch$att_check3b, ignore.case = T)))
...
T.selflaunch <- mutate(T.selflaunch, att1.success = ifelse(T.selflaunch$att_check1___1 == 1 & T.selflaunch$att_check1___4 == 1 & T.selflaunch$att_check1___5 == 1, 1, 0))
T.selflaunch$att1.success.factor <- factor(T.selflaunch$att1.success, levels = 0:1)
levels(T.selflaunch$att1.success.factor) <- c("first attention checks failed", "first attention checks successed")

T.selflaunch <- mutate(T.selflaunch, att2.success = ifelse(T.selflaunch$last_30_ecig_yn == 1 & T.selflaunch$att_check2 == 3, 1, ifelse(T.selflaunch$last_30_ecig_yn == 1, 0, NA)))
T.selflaunch$att2.success.factor <- factor(T.selflaunch$att2.success, levels = 0:1)
levels(T.selflaunch$att2.success.factor) <- c("second attention checks failed", "second attention checks successed")

T.selflaunch <- mutate(T.selflaunch, att3.success = ifelse(T.selflaunch$att_check3a == 4 & grepl("decision making", T.selflaunch$att_check3b, ignore.case = T), 1, 0))
T.selflaunch$att3.success.factor <- factor(T.selflaunch$att3.success, levels = 0:1)
levels(T.selflaunch$att3.success.factor) <- c("third attention checks failed", "third attention checks successed")

T.selflaunch$temp.att2 <- T.selflaunch$att2.success
T.selflaunch$temp.att2[is.na(T.selflaunch$att2.success)] = 1
sum.att.success <- T.selflaunch$att1.success + T.selflaunch$temp.att2 + T.selflaunch$att3.success
