---
title: "Pilot 1 - MTurk Concierge - Distribution of Survey Time"
author: "Yi"
date: "5/28/2020"
...
Among all participants in the Redcap (N = 227), 171 participants have valid Redcap survey time (from the beginning of "Work ID" section to the beginning of "Advertisements" section), and they distribute as below:
```{r}
# sum(!is.na(data$diff.time))
RC.mean <- mean(data$diff.time, na.rm = T)

ggplot(data = data, aes( x = redcapid, y = diff.time)) + geom_point(na.rm = T) + labs(y = "Redcap Survey Time (in secs)", title = "Pilot 1 - MTurk Concierge Redcap Survey Time")
```
...
ggplot(data = clean.data, aes( x = redcapid, y = diff.time)) + 
  geom_point(na.rm = T) + 
  labs(y = "Redcap Survey Time (in secs)", title = "Pilot 1 - MTurk Concierge Redcap Survey Time") + 
...
After merging the Redcap dataset and the Sawtooth dataset by redcapid, among all participants in the merged dataset (N = 227), 169 participants have valid information on Sawtooth survey time, and they distribute as below:
```{r}
ggplot(data = mergedata, aes(x = redcapid, y = md_time_inSec)) + 
  geom_point(na.rm = T) +
  labs(y = "Sawtooth Survey Time (in secs)", title = "Pilot 1 - MTurk Concierge Sawtooth Survey Time") 
```
