---
title: "Pilot 1 - PP Concierge - Distribution of Survey Time"
author: "Yi"
date: "5/28/2020"
...
```{r}
data <- read.csv("/Users/zhan039/Desktop/USC_HEAL/EVOC (CONJOINT)/04292020/PP.csv", na.strings = "")
```
...
Among all participants in the Redcap (N = 194), 191 participants have valid Redcap survey time (from the beginning of "Work ID" section to the beginning of "Advertisements" section), and they distribute as below:
```{r}
# sum(!is.na(data$diff.time))
RC.mean <- mean(data$diff.time, na.rm = T)

ggplot(data = data, aes( x = redcapid, y = diff.time)) + geom_point(na.rm = T) + labs(y = "Redcap Survey Time (in secs)", title = "Pilot 1 - PP Concierge Redcap Survey Time")
```

There's one extreme large value:
```{r}
RC.outliers <- data[which(data$diff.time > 15000), c("redcapid", "diff.time")]
...
After removing this participant (redcapid = 6035), the Redcap Survey Time distribute as below:
...
ggplot(data = clean.data, aes( x = redcapid, y = diff.time)) + 
  geom_point(na.rm = T) + 
  labs(y = "Redcap Survey Time (in secs)", title = "Pilot 1 - PP Concierge Redcap Survey Time (cleaned)") + 
...
Under the mean survey time from the cleaned Redcap dataset, the bottom 5% of the survey time are listed below:
```{r}
...
After merging the cleaned Redcap dataset and the Sawtooth dataset by redcapid, among all participants in the merged dataset (N = 193), 188 participants have valid information on Sawtooth survey time, and they distribute as below:
```{r}
ggplot(data = mergedata, aes(x = redcapid, y = md_time_inSec)) + 
  geom_point(na.rm = T) +
  labs(y = "Sawtooth Survey Time (in secs)", title = "Pilot 1 - PP Concierge Sawtooth Survey Time") 
```

It looks like there're two potential outliers, but not sure about it. All points are kept for now.
```{r}
