---
title: "09302019 demo Mturk self-launch"
author: "Yi"
date: "10/7/2019"
output: pdf_document
---
...
M.selflaunch <- data
dim(M.selflaunch)
colnames(M.selflaunch)
length(unique(M.selflaunch$redcapid))
```
...
# gender
FrequencyPercentage(M.selflaunch$gender.factor)

# Ethnicity
FrequencyPercentage(M.selflaunch$eth.factor)

# race
miss.race <- NROW(which(M.selflaunch$race___1 == 0 & M.selflaunch$race___2 == 0 & M.selflaunch$race___3 == 0 & M.selflaunch$race___4 == 0 & M.selflaunch$race___5 == 0 & M.selflaunch$race___6 == 0 & M.selflaunch$race___7 ==0))

total.race <- dim(M.selflaunch)[1] - miss.race

RaceDescrip <- function(race){
  frequency <- table(race)
  percentage <- round(frequency/total.race * 100, 1)
  return(rbind(frequency, percentage))
}

RaceDescrip(M.selflaunch$race___1.factor)

RaceDescrip(M.selflaunch$race___2.factor)

RaceDescrip(M.selflaunch$race___3.factor)

RaceDescrip(M.selflaunch$race___4.factor)

RaceDescrip(M.selflaunch$race___5.factor)

RaceDescrip(M.selflaunch$race___6.factor)

RaceDescrip(M.selflaunch$race___7.factor)

# Highest level of education
FrequencyPercentage(M.selflaunch$degree_program.factor)

# currently enrolled in a program
freq.degree.type <- table(M.selflaunch$degree_type.factor)
perc.degree.type <- round(freq.degree.type/sum(M.selflaunch$current_degree_program == 1, na.rm = T) * 100,1)
rbind(freq.degree.type, perc.degree.type)

# own or rent home
FrequencyPercentage(M.selflaunch$own_rent.factor)

# currently living situation
FrequencyPercentage(M.selflaunch$home.factor)

# current financial situation
FrequencyPercentage(M.selflaunch$overall_income.factor)

# childhood financial situation
FrequencyPercentage(M.selflaunch$family_income.factor)

# Country of residence
FrequencyPercentage(M.selflaunch$country.factor)
...
num.first.fail <- sum(!(M.selflaunch$att_check1___1 == 1 & M.selflaunch$att_check1___4 == 1 & M.selflaunch$att_check1___5 == 1))

num.second.fail <- sum(M.selflaunch$last_30_ecig_yn == 1 & M.selflaunch$att_check2 != 3, na.rm = T)

num.third.fail <- sum(!(M.selflaunch$att_check3a == 4 & grepl("decision making", M.selflaunch$att_check3b, ignore.case = T)))

num.first.fail
num.second.fail
num.third.fail

# attention checks' results
M.selflaunch <- mutate(M.selflaunch, att1.success = ifelse(M.selflaunch$att_check1___1 == 1 & M.selflaunch$att_check1___4 == 1 & M.selflaunch$att_check1___5 == 1, 1, 0))
M.selflaunch$att1.success.factor <- factor(M.selflaunch$att1.success, levels = 0:1)
levels(M.selflaunch$att1.success.factor) <- c("first attention checks failed", "first attention checks successed")

M.selflaunch <- mutate(M.selflaunch, att2.success = ifelse(M.selflaunch$last_30_ecig_yn == 1 & M.selflaunch$att_check2 == 3, 1, ifelse(M.selflaunch$last_30_ecig_yn == 1, 0, NA)))
M.selflaunch$att2.success.factor <- factor(M.selflaunch$att2.success, levels = 0:1)
levels(M.selflaunch$att2.success.factor) <- c("second attention checks failed", "second attention checks successed")

M.selflaunch <- mutate(M.selflaunch, att3.success = ifelse(M.selflaunch$att_check3a == 4 & grepl("decision making", M.selflaunch$att_check3b, ignore.case = T), 1, 0))
M.selflaunch$att3.success.factor <- factor(M.selflaunch$att3.success, levels = 0:1)
levels(M.selflaunch$att3.success.factor) <- c("third attention checks failed", "third attention checks successed")

M.selflaunch$temp.att2 <- M.selflaunch$att2.success
M.selflaunch$temp.att2[is.na(M.selflaunch$att2.success)] = 1
sum.att.success <- M.selflaunch$att1.success + M.selflaunch$temp.att2 + M.selflaunch$att3.success
FrequencyPercentage(sum.att.success)
```
