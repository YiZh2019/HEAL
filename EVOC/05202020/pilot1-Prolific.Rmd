...
colnames(table1) <- c(paste0("First", footnote_marker_alphabet(1)), paste0("Second", footnote_marker_alphabet(2)), paste0("Second", footnote_marker_alphabet(3)), "Third", paste0("Fourth", footnote_marker_alphabet(4)))
rownames(table1) <- c("Success", "Failure", "Missing", "Total")

kableExtra::kable(table1, escape = F, caption = "N(%) of Attention Check Questions Failures", align = "c") %>% footnote(alphabet = c("Variable name of the first attention check: att_check1 (the wheels question), only the full selection of option 1, 4, and 5 is seemed as success", "Among all participants", "Only among participants who saw it (last_30_ecig_yn = 1)", "Variable name of the fourth attention check: att_check_3b")) %>% kable_styling(bootstrap_options = "striped", full_width = F) %>% row_spec(2, bold = T)
```
...
### 8. Check for outliers of REDCap survey time with 2SD and mean completion time
```{r}
# outliers
...
### 10. Check for outliers of REDCap survey time 
```{r}
