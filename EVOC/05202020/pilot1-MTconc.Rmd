...
```{r}
data <- read.csv("/Users/zhan039/Desktop/USC_HEAL/EVOC (CONJOINT)/04292020/MTconc.csv", na.strings = "")
```
...
Among all participants in the Redcap (N = 227), 169 participants have valid information on bot status in the Maxdiff pilot 1 dataset. Among all participants in the Maxdiff pilot 1 dataset (N = 877), 230 participants are marked as "MTCo" in the variable "project_type". All analyses in this report are based on participants from Redcap. Participants whose redcapid are not in Redcap datasets are excluded. Therefore, the valid sample size is 169. (Let me know if you hope to analyze all participants marked as "MTCo" in the Maxdiff dataset)
...
# bot[which(bot$project_type == "MTCo"), ]
...
kableExtra::kable(ST.iqr.outliers, escape = F, caption = "Potential Outliers (defined by Q3 + 1.5 * IQR and Q1 - 1.5 * IQR)", align = "c") %>% kable_styling(bootstrap_options = "striped", full_width = F) 
