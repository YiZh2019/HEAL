...
table1 <- as.data.frame(cbind(att2_1, att2_2, att1, att3a, att3b_2))

colnames(table1) <- c(paste0("First", footnote_marker_alphabet(1)), paste0("First", footnote_marker_alphabet(2)), paste0("Second", footnote_marker_alphabet(3)), paste0("Third", footnote_marker_alphabet(4)), paste0("Fourth", footnote_marker_alphabet(5)))
rownames(table1) <- c("Success", "Failure", "Missing", "Total")

kableExtra::kable(table1, escape = F, caption = "N(%) of Attention Check Questions Failures", align = "c") %>% footnote(alphabet = c("Please select '3 to 5 days' as your answer. Percentages are calculated among all participants", "Percentages are calculated among participants who saw it (last_30_ecig_yn = 1)", "Which of the following objects have wheels? The full selection of option 1, 4, and 5 is seemed as success, any miss is seemed as a failure", "Please select the box marked 'Other' ", "Please specify 'Other' (Please type the words 'Decision making'). The logic of success or failure is case insensitive.")) %>% kable_styling(bootstrap_options = "striped", full_width = F) %>% row_spec(2, bold = T)
```
...
table2 <- as.data.frame(cbind(maxdiff1, maxdiff2, maxdiff3))
