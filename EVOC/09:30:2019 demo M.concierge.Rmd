...
M.concierge <- data
dim(M.concierge)
colnames(M.concierge)
length(unique(M.concierge$redcapid))
```
...
# gender
FrequencyPercentage(M.concierge$gender.factor)

# Ethnicity
FrequencyPercentage(M.concierge$eth.factor)

# race
miss.race <- NROW(which(M.concierge$race___1 == 0 & M.concierge$race___2 == 0 & M.concierge$race___3 == 0 & M.concierge$race___4 == 0 & M.concierge$race___5 == 0 & M.concierge$race___6 == 0 & M.concierge$race___7 ==0))

total.race <- dim(M.concierge)[1] - miss.race
...
RaceDescrip(M.concierge$race___1.factor)

RaceDescrip(M.concierge$race___2.factor)

RaceDescrip(M.concierge$race___3.factor)

RaceDescrip(M.concierge$race___4.factor)

RaceDescrip(M.concierge$race___5.factor)

RaceDescrip(M.concierge$race___6.factor)

RaceDescrip(M.concierge$race___7.factor)

# Highest level of education
FrequencyPercentage(M.concierge$degree_program.factor)

# currently enrolled in a program
freq.degree.type <- table(M.concierge$degree_type.factor)
perc.degree.type <- round(freq.degree.type/sum(M.concierge$current_degree_program == 1, na.rm = T) * 100,1)
rbind(freq.degree.type, perc.degree.type)

# own or rent home
FrequencyPercentage(M.concierge$own_rent.factor)

# currently living situation
FrequencyPercentage(M.concierge$home.factor)

# current financial situation
FrequencyPercentage(M.concierge$overall_income.factor)

# childhood financial situation
FrequencyPercentage(M.concierge$family_income.factor)

# Country of residence
FrequencyPercentage(M.concierge$country.factor)
...
num.first.fail <- sum(!(M.concierge$att_check1___1 == 1 & M.concierge$att_check1___4 == 1 & M.concierge$att_check1___5 == 1))

num.second.fail <- sum(M.concierge$last_30_ecig_yn == 1 & M.concierge$att_check2 != 3, na.rm = T)

num.third.fail <- sum(!(M.concierge$att_check3a == 4 & grepl("decision making", M.concierge$att_check3b, ignore.case = T)))
...
M.concierge <- mutate(M.concierge, att1.success = ifelse(M.concierge$att_check1___1 == 1 & M.concierge$att_check1___4 == 1 & M.concierge$att_check1___5 == 1, 1, 0))
M.concierge$att1.success.factor <- factor(M.concierge$att1.success, levels = 0:1)
levels(M.concierge$att1.success.factor) <- c("first attention checks failed", "first attention checks successed")

M.concierge <- mutate(M.concierge, att2.success = ifelse(M.concierge$last_30_ecig_yn == 1 & M.concierge$att_check2 == 3, 1, ifelse(M.concierge$last_30_ecig_yn == 1, 0, NA)))
M.concierge$att2.success.factor <- factor(M.concierge$att2.success, levels = 0:1)
levels(M.concierge$att2.success.factor) <- c("second attention checks failed", "second attention checks successed")

M.concierge <- mutate(M.concierge, att3.success = ifelse(M.concierge$att_check3a == 4 & grepl("decision making", M.concierge$att_check3b, ignore.case = T), 1, 0))
M.concierge$att3.success.factor <- factor(M.concierge$att3.success, levels = 0:1)
levels(M.concierge$att3.success.factor) <- c("third attention checks failed", "third attention checks successed")

M.concierge$temp.att2 <- M.concierge$att2.success
M.concierge$temp.att2[is.na(M.concierge$att2.success)] = 1
sum.att.success <- M.concierge$att1.success + M.concierge$temp.att2 + M.concierge$att3.success
